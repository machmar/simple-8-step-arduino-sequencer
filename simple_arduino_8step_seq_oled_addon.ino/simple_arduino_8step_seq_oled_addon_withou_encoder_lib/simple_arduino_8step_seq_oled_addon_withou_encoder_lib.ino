...
void rotate() {
  ...
  if (screenSelect == 0) {
    if (encoderAState == 1 && encoderBState == 0) {
      mainMenuSel++;
    } else if (encoderAState == 1 && encoderBState == 1){
      mainMenuSel--;
    }
  ...
  //if (encoderAState != encoderBState || encoderAState == encoderBState) screenSaverRST();
}
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(encoderA), screenSaverRST, RISING);
  encoderAState = digitalRead(encoderA);
  encoderBState = digitalRead(encoderB);
